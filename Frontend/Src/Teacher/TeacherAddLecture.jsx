...
const TeacherAddLecture = () => {
  ...
  const getButtonStyle = (active) =>
    `w-full p-4 text-sm font-bold rounded-xl shadow-md transition-all duration-300 
     ${active ? 'bg-yellow-500 text-white' : 'bg-yellow-100 text-black hover:bg-yellow-200'}`;

  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-50 to-white p-6 md:p-12 text-black ">
      {/* Hero Section */}
      <div className="text-center mb-10">
        <h1 className="text-4xl font-extrabold text-black mb-2 mt-10">🎓 Welcome, Faculty</h1>
        <p className="text-gray-700">Manage your courses, quizzes, lectures and more</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-xl rounded-3xl p-8 mb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="text-sm font-semibold text-black mb-1 block">📘 Select Course</label>
            <select
          ...
          <div>
            <label className="text-sm font-semibold text-black mb-1 block">🏫 Select Batch</label>
            <select
              value={selectedBatch}
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8 ">
        <button onClick={() => setActiveTab('lectures')} className={getButtonStyle(activeTab === 'lectures')}>
          🎬 Add Lecture
        </button>
      </div>
